...
describe('client', () => {
  ...
  describe('initialization', () => {
    ...
    test('Retrieves token and user from storage and sets it', async () => {
      ...
      expect(client.accessToken).toEqual(mockResponseToken.access_token)

      storage.getItem = _getItem
    })
    test('Refreshes token and user when initialized', async () => {
      const newUser = { ...mockUser, username: 'not-admin@ixortalk.com' }
      const newTokenResponse = {
        ...mockResponseToken,
        access_token: 'new accesstoken',
      }
      const _getItem = storage.getItem
      fetchMock.post(endpoint(`/uaa/oauth/token`), newTokenResponse)
      fetchMock.get(endpoint(`/uaa/user`), newUser)
      storage.getItem = jest.fn(async () =>
        JSON.stringify({
          user: mockUser,
          token: createToken(mockResponseToken),
      ...
      expect(client.currentUser).toEqual(newUser)
      expect(client.accessToken).toEqual(newTokenResponse.access_token)

      storage.getItem = _getItem
    })

    test('Sets old token and user when refreshing rejects', async () => {
      const _getItem = storage.getItem
      fetchMock.postOnce(endpoint(`/uaa/oauth/token`), { status: 500 })
      fetchMock.getOnce(endpoint(`/uaa/user`), { status: 500 })
      storage.getItem = jest.fn(async () =>
        JSON.stringify({
          user: mockUser,
          token: createToken(mockResponseToken),
  ...
  describe('logging out', () => {
    ...
    test.only('logs out automatically in case of a 401 and failed token refresh', async () => {
      ...
      fetchMock
        ...
        .post(endpoint('/uaa/oauth/token'), 401, { overwriteRoutes: true })
  ...
  describe('me', () => {
    beforeEach(reinitialize)
    test('resolves into a javascript object containing the user upon success', async () => {
      fetchMock.get(`${config.baseUrl}/uaa/user`, mockUser)
      const user = await client.me()
      expect(user).toEqual(mockUser)
    })
    test('rejects into a "FetchError".', async () => {
      fetchMock.get(`${config.baseUrl}/uaa/user`, 401)
      let error
      try {
        await client.me()
      } catch (e) {
  ...
  describe('onAuthChange', () => {
    ...
    test('Correctly fires callbacks when a log-in occurs', async () => {
      ...
      expect(handler.mock.calls[1]).toEqual([mockUser])
    })
  ...
  describe('tokens', () => {
    ...
    test('throws an error when refreshing the token fails', async () => {
      const body = { test: 'test' }
      await loginAndMockLogin()
      fetchMock.restore()
      fetchMock.get(
      ...
      fetchMock.post(endpoint(`/uaa/oauth/token`), 401)
