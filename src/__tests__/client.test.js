...
describe('client', () => {
  ...
  describe('logging in', () => {
    ...
    test('logs in correctly', async () => {
      ...
      const user = await client.logIn({email: 'admin@ixortalk.com', password: 'admin'})
      
      const [tokenEndpoint, tokenOpts] = fetchMock.lastCall(`${config.baseUrl}/uaa/oauth/token`, 'POST')
      ...
      expect(tokenOpts.headers['Authorization']).toEqual(basicAuth(config.clientId, config.clientSecret))
      expect(
      ...
      const [userEndpoint, userOpts] = fetchMock.lastCall(`${config.baseUrl}/uaa/user`, 'GET')
      expect(userOpts.headers['Authorization']).toEqual(`Bearer ${mockResponseToken.access_token}`)
    })
  })
  describe('logging out', () => {
    ...
    test('Throws an error when not logged in', async () => {
      ...
      expect(error)
        .toEqual(new Error('You are not logged in at this time.'))
    })
    ...
    test('logs out correctly', async () => {
      ...
      ).toHaveBeenCalledWith('IxorTalk Client')
      expect(
        fetchMock.called(`${config.baseUrl}/logout`)
      ).toEqual(true)
    })
  })
  describe('Registering', () => {
    ...
    test('does the user registration call correctly', async () => {
      ...
      const user = {email: 'admin@ixortalk.com', firstName: 'admin', lastName: 'admin', langKey: 'en'}
      await client.register(user)
      
      const [endpoint, opts] = fetchMock.lastCall(`${config.baseUrl}/user-registration`, 'POST')
      expect(
      ...
      expect(
        ...
        {username: user.email, firstName: user.firstName, lastName: user.lastName, langKey: user.langKey}
      )
    })
  })
  ...
  describe('confirming password', () => {
    ...
    test('successfully set the password', async () => {
      fetchMock.post(`${config.baseUrl}/uaa/api/account/reset_password/finish`, 200)
      
      await client.confirmPassword({key: '123-this-is-a-key', password: 'admin'})
      
      const [endpoint, opts] = fetchMock.lastCall(`${config.baseUrl}/uaa/api/account/reset_password/finish`, 'POST')
      
      expect(JSON.parse(opts.body))
        .toEqual({key: '123-this-is-a-key', newPassword: 'admin'})
    })
  })
  describe('reset password', () => {
    ...
    test('successfully request a reset', async () => {
      fetchMock.post(`${config.baseUrl}/uaa/api/account/reset_password/init`, 200)
      
      await client.resetPassword({email: 'admin@ixortalk.com'})
      
      const [endpoint, opts] = fetchMock.lastCall(`${config.baseUrl}/uaa/api/account/reset_password/init`, 'POST')
      expect(JSON.parse(opts.body))
        .toEqual({email: 'admin@ixortalk.com'})
    })
  })
  ...
  describe('onAuthChange', () => {
    ...
    test('Correctly fires callbacks when a log-in occurs', async () => {
      ...
      expect(fetchMock.called(`${config.baseUrl}/uaa/oauth/token`))
        .toEqual(true)
      expect(handler.mock.calls[0])
        .toEqual([mockUser])
    })
    ...
    test('Correctly fires callbacks when a log-out occurs', async () => {
      ...
      expect(fetchMock.called(`${config.baseUrl}/logout`))
        .toEqual(true)
      expect(handler.mock.calls[0])
        .toEqual([null])
    })
  ...
  describe('fetch', () => {
    ...
    test('Fetches from an endpoint of the configurated platform and returns a "Response" object.', async () => {
      ...
      expect(fetchMock.called(`${config.baseUrl}/my-resource`))
        .toEqual(true)
    ...
    test('Does a "POST"-request with body', async () => {
      ...
      const result = await client.fetch('/my-resource', {method: 'POST', body})
      
      const [endpoint, opts] = fetchMock.lastCall(`${config.baseUrl}/my-resource`, 'POST')
    ...
    test('Injects the token if someone is logged in.', async () => {
      ...
      const [endpoint, opts] = fetchMock.lastCall(`${config.baseUrl}/my-auth-resource`)
      expect(opts.headers['Authorization'])
        .toEqual(`Bearer ${createToken(mockResponseToken).accessToken}`)
  ...
  describe('Fetch shorthands', () => {
    ...
    describe('get', () => {
      test('does a request with method "GET"', async () => {
        ...
        const [endpoint, opts] = fetchMock.lastCall(`${config.baseUrl}/objects/someId`)
        expect(opts.method)
          .toEqual('GET')
        expect(resultBody).toEqual(body)
      })
    ...
    describe('post', () => {
      test('does a request with method "POST".', async () => {
        ...
        const [endpoint, opts] = fetchMock.lastCall(`${config.baseUrl}/objects`)
